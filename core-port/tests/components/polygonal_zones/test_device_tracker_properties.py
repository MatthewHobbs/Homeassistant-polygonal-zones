...
from homeassistant.components.polygonal_zones.device_tracker import (
    PolygonalZoneEntity,
    async_setup_entry,
)
...
async def test_async_setup_entry_no_download(hass_with_setup) -> None:
    ...
    with (
        patch(
            "homeassistant.components.polygonal_zones.device_tracker.entity_platform.async_get_current_platform",
            return_value=platform,
        ),
        patch(
            "homeassistant.components.polygonal_zones.device_tracker.generate_entity_id",
...
async def test_async_setup_entry_download_creates_local_path(hass_with_setup, tmp_path) -> None:
    ...
    with (
        ...
        patch(
            "homeassistant.components.polygonal_zones.device_tracker.download_zones",
            new=AsyncMock(),
        ) as download_mock,
    ):
        await async_setup_entry(hass, entry, add_entities)
