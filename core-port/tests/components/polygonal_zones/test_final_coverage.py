...
from homeassistant.components.polygonal_zones.device_tracker import PolygonalZoneEntity
...
async def test_retry_callback_fires_initialize_again() -> None:
    ...
    with (
        patch(
            "homeassistant.components.polygonal_zones.device_tracker.async_at_started",
            side_effect=fake_at_started,
        ),
        patch(
            "homeassistant.components.polygonal_zones.device_tracker.get_zones",
            new=AsyncMock(side_effect=RuntimeError("boom")),
        ),
        patch(
            "homeassistant.components.polygonal_zones.device_tracker.async_call_later",
            side_effect=fake_call_later,
        ),
        patch("homeassistant.components.polygonal_zones.device_tracker.ir.async_create_issue"),
        patch("homeassistant.components.polygonal_zones.device_tracker.ir.async_delete_issue"),
    ):
...
async def test_public_only_resolver_drops_private_keeps_public() -> None:
    ...
    from homeassistant.components.polygonal_zones.utils.general import _PublicOnlyResolver
...
def test_event_should_trigger_attribute_change_returns_true() -> None:
    ...
    from homeassistant.components.polygonal_zones.utils.general import event_should_trigger
...
def test_parse_zone_collection_string_features_rejected() -> None:
    ...
    from homeassistant.components.polygonal_zones.services.helpers import parse_zone_collection
...
async def test_unload_entry_path_traversal_falls_back(tmp_path, monkeypatch) -> None:
    ...
    from homeassistant.components.polygonal_zones import async_unload_entry
    from homeassistant.components.polygonal_zones.const import DOMAIN
    ...
    with patch(
        "homeassistant.components.polygonal_zones.safe_config_path",
